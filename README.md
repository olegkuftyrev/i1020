...
1. **users**
   ...
   - `name` - Имя пользователя (varchar 255, nullable)
   - `email` - Email (varchar 254, not null, unique)
   - `role` - Роль пользователя (enum: 'associate', 'manager', 'admin', default: 'associate')
   - `password` - Хеш пароля (varchar 255, not null, хешируется через scrypt)
   - `created_at` - Дата создания (timestamp, auto-create)
   - `updated_at` - Дата обновления (timestamp, auto-update)

2. **remember_me_tokens**
   - `id` - Primary key (integer, auto-increment)
   - `tokenable_id` - Foreign Key на users(id) (integer, cascade delete)
   - `hash` - Хеш токена "запомнить меня" (varchar 255, unique)
   - `created_at` - Дата создания (timestamp)
   - `updated_at` - Дата обновления (timestamp)
   - `expires_at` - Дата истечения токена (timestamp)

### Модель User

Модель User (`app/models/user.ts`) предоставляет типизированный доступ к данным пользователя через Lucid ORM.
...
export default class User extends compose(BaseModel, AuthFinder) {
  ...
  @column()
  declare name: string | null // Имя пользователя (может быть пустым)

  @column()
  declare email: string // Email (обязательное, уникальное)

  @column()
  declare role: 'associate' | 'manager' | 'admin' // Роль пользователя

  @column({ serializeAs: null })
  declare password: string // Хеш пароля (не сериализуется в JSON)

  @column.dateTime({ autoCreate: true })
  declare createdAt: DateTime // Автоматически создается

  @column.dateTime({ autoCreate: true, autoUpdate: true })
  declare updatedAt: DateTime | null // Автоматически обновляется
}
```

#### Особенности модели

- **AuthFinder Mixin** - Встроенная аутентификация:
  - Автоматический поиск пользователя по email
  - Хеширование пароля через scrypt при сохранении
  - Метод `User.verifyCredentials(email, password)` для проверки учетных данных

- **Remember Me токены** - Поддержка через `DbRememberMeTokensProvider`:
  - Автоматическое управление токенами "запомнить меня"
  - Связь с таблицей `remember_me_tokens`

- **Безопасность**:
  - Пароль не включается в JSON ответы (`serializeAs: null`)
...
const user = await User.create({
  ...
  role: 'associate', // или 'manager', 'admin'
  password: 'plain_password', // Автоматически захешируется
})
```
...
```typescript
const user = await User.verifyCredentials('john@example.com', 'password')
// Выбрасывает ошибку если учетные данные неверны
```
...
```typescript
await user.delete()
// Remember Me токены удаляются автоматически (cascade delete)
```
...
#### 1. **Associate** (Ассоциированный пользователь)

- **Описание**: Базовая роль по умолчанию для всех новых пользователей
- **Доступ**: Стандартный функционал приложения
- **Использование**: Обычные пользователи системы

#### 2. **Manager** (Менеджер)

- **Описание**: Расширенная роль с дополнительными правами
- **Доступ**: Управленческие функции и расширенный функционал
- **Использование**: Руководители, менеджеры проектов

#### 3. **Admin** (Администратор)

- **Описание**: Полный доступ к системе
- **Доступ**: Все функции, включая административные
- **Использование**: Системные администраторы

#### Управление ролями

**Изменение роли через профиль:**

- Пользователи могут изменять свою роль на странице `/settings/profile`
- Доступен выпадающий список с опциями: Associate, Manager, Admin
- Изменения сохраняются автоматически при выборе новой роли
...
- Роль валидируется на уровне базы данных (enum)
- Валидатор `updateProfileValidator` проверяет, что роль является одним из допустимых значений
- Значение по умолчанию: `'associate'`
